<template>
  <div class="flex bg-gray-100">
        ...
        <div class="flex">
          ...
          <div class="w-1/2 flex space-x-2 justify-end items-center px-3">
            <div class="flex space-x-2 justify-end items-center">
              ...
              <Button icon="pi pi-cog" class="p-button-sm p-button-secondary" @click="displayNav = !displayNav" />
            </div>
          </div>
        </div>
      </div>
      <div class="w-full overflow-x-auto px-5 reports-scroll" style="height: calc(100vh - 8rem)" :style="
        displayNav != true
          ? 'width: calc(100vw'
        ...
        <div class="h-full mx-auto border my-5 h-auto" style="width: 1024px">
          <div class="w-full bg-white py-5">
            <div class="flex">
              <div class="w-1/4">
                <div class="p-3">
                  <div v-if="editInfo" class="flex justify-start items-center space-x-2">
                    <Checkbox v-model="showLogo" :binary="true" id="show_logo" />
                    <label class="text-xs" for="show_logo">Show Logo</label>
                  </div>
                  <img v-if="showLogo" :src="report_header.report_company_preference.logo_url" alt="" />
                </div>
              </div>
              <div class="w-2/4">
                <div class="w-full flex flex-col justify-center items-center">
                  <InputText v-if="editInfo" type="text" class="p-inputtext-sm font-khmer text-sky-500 w-96" v-model="Company_name" />
                  <h1 v-else class="text-xl font-bold">
                    {{ Company_name }}
                  </h1>
                  <div class="py-2">
                    <InputText v-if="editInfo" type="text" class="p-inputtext-sm text-sky-500 w-56" v-model="Reporttype" />
              ...
              <div class="w-1/4 flex justify-end items-end">
                <Button v-if="editInfo" label="Save Update" icon="pi pi-check-circle" class="p-button-info p-button-sm" @click="editInfo = false" />
                <!-- <Button
                v-if="editInfo"
                icon="pi pi-bookmark"
                label="Save"
          ...
          <TreeTable :value="baseData" class="p-treetable-sm" style="margin-bottom: 2rem" :resizableColumns="true" :expandedKeys="expandedKeys" :metaKeySelection="true" @node-expand="expand" @node-collapse="collapse" @node-select="seclected" selectionMode="single" v-model:selectionKeys="key">
    ...
    <div class="w-96 bg-gray-50 h-auto overflow-hidden relative flex flex-col justify-between" v-show="displayNav">
      <div class="option">
        <div class="py-3 text-left px-10">
          <h2 class="font-bold text-xl">Option Report
            <span class="font-bold text-sky-600 text-xl">Profit and Lost</span>
          </h2>
        </div>
        <div class="report_period flex flex-col px-10 justify-start items-start">
          <p class="text-sm py-2">Report period</p>
          <Dropdown v-model="selectedPeriod" :options="periodData" class="w-64" optionLabel="name" optionValue="code" placeholder="Select reports period" />
          <p class="text-sm py-2">From</p>
          <Calendar id="basic" v-model="date1" class="w-64" autocomplete="off" />
        ...
        <div class="report_period flex flex-col px-10 justify-start items-start">
          ...
          <Dropdown v-model="selectedGroupedCity" :options="groupedCities" optionLabel="label" class="w-64" optionGroupLabel="label" optionGroupChildren="items">
            <template>
