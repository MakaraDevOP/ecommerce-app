    ...
    <div class="border rounded  ">
      <div class="h-[calc(100vh-4.1rem)]">
        <DataTable ref="dt" :value="contacts" @row-dblclick="doubleClick" v-model:selection="selectContact" selectionMode="single" dataKey="id" :paginator="true" :maximizable="true" :modal="true" scrollHeight="flex" :scrollable="true" scrollDirection="both" showGridlines stripedRows responsiveLayout="scroll" class="p-datatable-sm text-xs" style="font-size:12px !important" :rows="10" paginatorTemplate="CurrentPageReport FirstPageLink PrevPageLink PageLinks NextPageLink LastPageLink RowsPerPageDropdown" :rowsPerPageOptions="[10, 20, 50]" currentPageReportTemplate="Showing {first} to {last} of {totalRecords}">
          ...
          <Column field="title" header="Title" style="flex-grow: 1; flex-basis: 50px;"></Column>
          <Column field="full_name" header="Full Name" style="flex-grow: 1; flex-basis: 50px;"></Column>
          <Column field="phone" header="Phone" style="flex-grow: 1; flex-basis: 50px;"></Column>
          <Column field="email" header="Email" style="flex-grow: 1; flex-basis: 50px;"></Column>
      ...
      <Dialog header="Contact" v-model:visible="showDialog" :breakpoints="{ '960px': '75vw', '640px': '100vw' }" :style="{ width: '50vw' }">
        <div class="py-2 grid grid-cols-2 gap-4">
          <div class="flex flex-col space-y-4">
            <InputText v-model="contact.full_name" placeholder="product name" />
            <InputText v-model="contact.phone" placeholder="product name" />
            <InputText v-model="contact.email" placeholder="product name" />
            <Dropdown v-model="contact.title" :options="productType" optionLabel="name" optionValue="name" />
            <Dropdown v-model="contact.company_id" :options="dataCompany" optionLabel="name" optionValue="name" />
