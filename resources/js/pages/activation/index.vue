		...
		<div class="border rounded  ">
			<div class="h-[calc(100vh-4rem)]">
				<DataTable
					...
					v-model:selection="selectActive"
					...
					<template #header>
						<div class="flex justify-between items-center">
							<div class="text-left flex space-x-2  items-center">
								<div class="text-left text-lg">Sales</div>
								<div class="py-1">
									<InputText
										placeholder="Search"
										type="text"
            ...
            <template #body="slot">
              <div class="flex space-x-2">
                ...
                <Button icon="pi pi-trash" class="p-button-sm p-button-danger" @click="destroy(slot.data.id)" />
					...
					<template #empty>
						<div class="w-full text-center"> No Sales found. 😋</div>
					</template>
				</DataTable>
			</div>
			<!-- DIALOG SIDEBAR ------------------------------------------------------------------------------------------------------------->
			...
			>
				...
				>
					<div class="px-2">
						<div>
							<div class="text-lg font-bold py-2 top-0 fixed">Sale Detail</div>
							<div class="grid grid-cols-6 gap-2 py-2">
								<div class="col-span-2">
									<div class="text-md font-semibold p-1">Customer Name</div>
									<Dropdown
										v-model="activation.customer_id"
										:options="customers"
										optionLabel="company_name"
										optionValue="id"
										placeholder="select company "
										class="w-full"
										:disabled="activation.id != null"
									/>
								</div>
								<div>
									<div class="text-md font-semibold p-1">Is Active</div>
									<Dropdown
										v-model="activation.is_active"
										:options="status"
										optionLabel="name"
										optionValue="code"
										placeholder="status"
							...
							<div>
								<div class="h-[400px]">
									<DataTable
										ref="dt"
										:value="activation.activation_line"
										editMode="cell"
									...
									>
										...
										<Column
											field="product_id"
											header="Product"
											style="flex-grow: 0; flex-basis: 400px;"
										>
										...
										<Column
											field="qty"
											header="QTY"
											style="flex-grow: 0; flex-basis: 220px;"
										>
											<template #body="{ data, field }">
										...
										>
											<template #body="slot">
												<InputText
													v-model="slot.data.period"
													autofocus
													@keyup="changeDate(slot.index, slot.data.term_id, '', true)"
													@change="changeDate(slot.index, slot.data.term_id, '', true)"
													class="w-full"
													type="number"
										...
										>
											<template #body="slot">
												<Calendar
													inputId="icon"
													v-model="slot.data.activated_date"
													@date-select="changeDate(slot.index, slot.data.term_id, 'activated', false)"
													:showIcon="false"
													autofocus
													class="w-full"
													dateFormat="dd-mm-yy"
										...
										>
											<template #body="slot">
												<Calendar
													inputId="icon"
													v-model="slot.data.expired_date"
													:showIcon="false"
													@date-select="changeDate(slot.index, slot.data.term_id, 'expired', false)"
													autofocus
													class="w-full"
													dateFormat="dd-mm-yy"
												/>
										...
										<Column
											field="Status"
											header="Status"
											style="flex-grow: 1; flex-basis: 150px; text-align: right;"
										>
											<template #body="slot">
												<div class="flex space-x-2 w-full text-right">
													<span
														class="text-[11px] inline-block py-1 px-2.5 leading-none text-center whitespace-nowrap align-baseline font-bold bg-gray-200 text-gray-700 rounded-full"
														v-if="slot.data.status == '2'"
													><span v-if="slot.data.id">Not Delivered</span><span v-else>New</span> </span>
													<span
														class="text-[11px] inline-block py-1 px-2.5 leading-none text-center whitespace-nowrap align-baseline font-bold bg-green-200 text-green-700 rounded-full"
										...
										>
											<template #body="slot">
												<div class="flex space-x-2 items-center justify-center w-full">
													<div
														@click="setActlineId(slot.data.id, slot.data.status)"
														v-if="slot.data.id"
													>
														<Button
																	...
																	>
																		<div class="flex justify-sart mb-2">
																			<div class="block  w-auto">
																				...
																				>
																					<div
																						class="flex border  rounded-md shadow  px-4 py-3"
																						:class="data.user_id == userID ? 'bg-teal-50 border-teal-200' : 'bg-gray-50 border-gray-400'"
																					>
																						<div class="py-1 grow-0 "><svg
			...
			<Dialog
				header="Line Note"
				v-model:visible="noteChateDialog"
...
export default {
	...
	methods: {
		...
		//Cange QTY
		changeQTY(data) {
			const product = this.products.find((ob) => ob.id == data.product_id);
